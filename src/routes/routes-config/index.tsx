...
const routeConfigToRouteObj = <T extends PageLayoutObject>(
  { authenticated = undefined, wrapper = undefined, ...route_props }: RouteConfig<T>,
  page_layout_map: T,
  ...
  if (hasElement && authenticated) {
    const original_element = route_props.element;
    route_props.element = <AuthChecker fallback={fallback_route}>{original_element}</AuthChecker>;
  }
...
export const createRoutes = <PageLayouts extends PageLayoutObject>({
  ...
  const FALLBACK_ROUTE = fallback_route || '/login'; // route we redirect to if not logged in
